...
class OrderItemForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.part_id.choices = [('', 'Selecione uma peça (opcional)')] + [
            (part.id, f"{part.name} - R$ {(part.selling_price or 0):.2f}".replace('.', ',')) 
        ...
        self.stock_item_id.choices = [('', 'Selecione um item de estoque (opcional)')] + [
...
class StockItemForm(FlaskForm):
    ...
    price = DecimalField('Preço Unitário (R$)', validators=[Optional()], places=2, default=0)
