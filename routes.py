...
def register_routes(app):
    ...
    def get_service_order_modal_data(id):
        ...
        order_data = {
            ...
            'km_inicial': f"{service_order.km_inicial:.2f}" if service_order.km_inicial is not None else 'Não informado',
            'km_final': f"{service_order.km_final:.2f}" if service_order.km_final is not None else 'Não informado',
            'km_total': f"{service_order.km_total:.2f}" if service_order.km_total is not None and service_order.km_total > 0 else 'Não informado',
            'km_rate': f"{service_order.km_rate:.2f}" if service_order.km_rate is not None else 'Não informado',
            'km_value': f"{service_order.km_value:.2f}" if service_order.km_value is not None else 'Não informado',
            'labor_value': f"{service_order.labor_value:.2f}" if service_order.labor_value is not None else 'Não informado',
            'parts_value': f"{service_order.parts_value:.2f}" if service_order.parts_value is not None else 'Não informado',
    ...
    def print_equipment_history(id):
        ...
        service_orders = ServiceOrder.query.join(
            equipment_service_orders
        ).filter(
            equipment_service_orders.c.equipment_id == id
